...
constraints_aux (AVar x) (v:vs) gamma = let Just w = lookup x gamma
                                        in ([Equ (Variable v) (Variable w)], vs)
constraints_aux (AApp t s) (x:(y:xs)) gamma = let (es1, v:vs) = constraints_aux t (y:xs) gamma
                                                  (es2, ws) = constraints_aux s (v:vs) gamma
                                    in (es1 ++ es2 ++ [Equ (Variable y) (FuncSym "arr" [Variable v, Variable x])], ws)
constraints_aux (ALam x s t) (y:(z:xs)) gamma = let (es, vs) = constraints_aux t (z:xs) ((x,s):gamma)
